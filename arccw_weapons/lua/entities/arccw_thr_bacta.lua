...
local function RegenerateHealth(ply)
    if IsValid(ply) and ply:Alive() then
        ...
        local regenAmount = math.random( 7, 13 ) --edit this for heal amount
        
        if currentHealth < maxHealth then
            ply:SetHealth(math.min(currentHealth + regenAmount, maxHealth)) --DON'T edit this line, edit above (ln 26)
        end
    end
end
...
function ENT:Initialize()
	if SERVER then
		self:SetModel("models/arccw/meeks/bacta_grenade.mdl") 
