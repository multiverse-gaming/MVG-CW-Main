	...
	if alt < 600 or ceiling < 600 or WallDist < (MinDist * 3 * (math.deg( math.acos( math.Clamp( Vector(0,0,1):Dot( myDir ) ,-1,1) ) ) / 180) ^ 2) then
		...
		if IsValid( self:GetHardLockTarget() ) then
			...
			if alt > mySpeed then
				...
				if not IsValid( self._LastAITarget ) or not self:AITargetInFront( self._LastAITarget, 180 ) or not self:AICanSee( self._LastAITarget ) then
					Target = self:AIGetTarget()
				end

				if IsValid( Target ) then
					if self:AITargetInFront( Target, 180 ) then
