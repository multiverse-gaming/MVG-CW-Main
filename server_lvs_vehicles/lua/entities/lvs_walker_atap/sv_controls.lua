...
function ENT:CalcThrottle( ply, cmd )
	local MoveSpeed = cmd:KeyDown( IN_SPEED ) and 170 or 125
...
function ENT:StartCommand( ply, cmd )
	...
	if (self:GetCannonMode() == 1) then return end
...
function ENT:GetAlignment( ent, phys )
	...
	local P = (self:GetCannonMode() == 1 and 0 ) or math.cos( math.rad(Move*2) )
	local R = (self:GetCannonMode() == 1 and 0 ) or math.cos( math.rad(Move) )
