...
function ENT:DrawTurretDriver()
	...
	local Pos,Ang = LocalToWorld( Vector(0,0,0), Angle(190,-90,0), Att.Pos, Att.Ang )
...
function ENT:LVSHudPaint( X, Y, ply )
	...
	local _,_, InRange-- = (self:GetCannonMode() == 1 and self:GetMainAimAngles()) or self:GetBottomAimAngles()

	if (self:GetDriverGunAngles() == 1) then
		_,_, InRange = self:GetMainAimAngles()
	else
		_,_, InRange = self:GetBottomAimAngles()
	end
