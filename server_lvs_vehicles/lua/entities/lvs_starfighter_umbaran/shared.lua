...
ENT.PrintName = "Umbaran"
ENT.Author = "Lustick2K"
ENT.Information = "Starfighter of the Unbaran Military"
ENT.Category = "[LVS] - CIS Vehicles"

ENT.Spawnable			= true
ENT.AdminSpawnable		= false

ENT.MDL = "models/helios/ship_umbaran1/ship_umbaran1.mdl"
ENT.GibModels = {
...
function ENT:InitWeapons()
	...
	weapon.Attack = function( ent )
		local bullet = {}
		bullet.Src 	= ent:LocalToWorld( Vector(197.128,0,159.643) )
	...
	weapon.Ammo = 6
	weapon.Delay = 00.1 -- this will turn weapon.Attack to a somewhat think function
	weapon.HeatRateUp = -0.5 -- cool down when attack key is held. This system fires on key-release.
	weapon.HeatRateDown = 0.25
	weapon.Attack = function( ent )
		local T = CurTime()

		if IsValid( ent._plazmatorpedo_small ) then
			if (ent._nextMissleTracking or 0) > T then return end

			ent._nextMissleTracking = T + 0.1 -- 0.1 second interval because those find functions can be expensive

			ent._plazmatorpedo_small:FindTarget( ent:GetPos(), ent:GetForward(), 30, 7500 )
		...
		local Pos = Vector( 48.67, (ent._swapMissile and -78.019 or 78.019), 63 )

		local Driver = self:GetDriver()

		local projectile = ents.Create( "lvs_plazmatorpedo_small" )
