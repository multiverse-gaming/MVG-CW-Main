...
function ENT:GetGunnerAimAng( ent, base, RearEnt )
	...
	local _, Ang = WorldToLocal( Pos, wAng, Pos, self:LocalToWorldAngles( Angle(0,180,0) ) )

	return Ang, trace.HitPos, (Ang.p < 30 and Ang.p > -40 and math.abs( Ang.y ) < 60)
end
...
function ENT:InitGunner()
	...
	weapon.Attack = function( ent )
		...
		local _, AimPos, InRange = base:GetGunnerAimAng( ent, base )

		if not InRange then return true end
	
		local ID1 = self:LookupAttachment( "gunBLeft" )
		local ID2 = self:LookupAttachment( "gunBRight" )
		...
		self.SNDSecondaryPrimary:PlayOnce( 100 + math.cos( CurTime() + ent:EntIndex() * 1337 ) * 5 + math.Rand(-1,1), 1 )
	end
	weapon.OnThink = function( ent, active )
		...
		local Ang, HitPos, InRange = base:GetGunnerAimAng( ent, base )

		local p = math.Clamp(Ang.p, -30, 23)
		local y = math.Clamp(Ang.y, -60, 60)
		-- local y = math.Clamp(AimAngles.y, -55, 55)

		-- ent:ManipulateBoneAngles(1,Angle(0,-p,-y))
		-- ent:ManipulateBoneAngles(3,Angle(0,p,y))

		self:ManipulateBoneAngles(2,Angle(0,y,-p))
		self:ManipulateBoneAngles(4,Angle(0,y,-p))
		-- self:ManipulateBoneAngles(4,Angle(0,Ang.y,math.Clamp(Ang.p,-10,30)))

		-- print(Ang)

		-- self:ManipulateBoneAngles(4,Angle(0,0,0))
