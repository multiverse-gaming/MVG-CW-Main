...
function ENT:Initialize()
	...
	self.Entity:SetNetworkedString( 99, "" .. sirensquadnames[sirensquadnumber] .. "" )
...
local ent = ents.Create( "republic_commando" )
ent:SetPos( SpawnPos )
ent:Spawn()
ent:Activate()
return ent
...
local squad = self:GetNetworkedString( 99 )

self.Entity:EmitSound("Oogway/republic_commando.mp3",600)

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Republic Commando activated")
end
...
self.Entity:StopSound("Oogway/republic_commando.mp3")

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Republic Commando deactivated")
...
for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Republic Commando removed")
ply:SendLua([[RunConsoleCommand("stopsound")]])
end
