...
local ent = ents.Create( "bad_batch_theme" )
ent:SetPos( SpawnPos )
ent:Spawn()
ent:Activate()
return ent
...
self.Entity:EmitSound("Oogway/bad_batch_theme.mp3",600)

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Bad Batch Theme activated")
end
...
self.Entity:StopSound("Oogway/bad_batch_theme.mp3")

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Bad Batch Theme deactivated")
...
for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Bad Batch Theme removed")
ply:SendLua([[RunConsoleCommand("stopsound")]])
end
