...
local ent = ents.Create( "death_of_a_master" )
ent:SetPos( SpawnPos )
ent:Spawn()
ent:Activate()
return ent
...
self.Entity:EmitSound("mvg_music/death_of_a_master.mp3",600)

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Death of a Master activated")
end
...
self.Entity:StopSound("mvg_music/death_of_a_master.mp3")

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Death of a Master deactivated")
...
for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Death of a Master removed")
ply:SendLua([[RunConsoleCommand("stopsound")]])
end
