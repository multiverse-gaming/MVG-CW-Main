...
ENT.ExplodeSound = "lfs/hyenabomber/rockets/rocket_explosion" .. math.random(1,11) .. ".wav"
...
if SERVER then
	...
	function ENT:SpiralFire()
		...
		if IsValid( pObj ) then
			...
			AF.p = math.Clamp( AF.p * 0.15,-90,90 )
			AF.y = math.Clamp( AF.y * 0.15,-90,90 )
			AF.r = math.Clamp( AF.r * 0.15,-90,90 )
			
			local AVel = pObj:GetAngleVelocity()
			pObj:AddAngleVelocity( Vector(AF.r,AF.p,AF.y) - AVel )
			pObj:SetVelocityInstantaneous(self:GetForward() *(self:GetStartVelocity() +5000))
		end
	end
	...
	function ENT:FollowTarget( followent )
		local speed = self:GetStartVelocity() + (self:GetDirtyMissile() and 5000 or 4000)
		...
		local pos = TargetPos + followent:GetVelocity() * 0.45
		...
		if IsValid( pObj ) then
			if not self:GetDisabled() then
				...
				local AF = self:WorldToLocalAngles( targetdir:Angle() )
				AF.p = math.Clamp( AF.p * 900,-turnrate,turnrate )
				AF.y = math.Clamp( AF.y * 900,-turnrate,turnrate )
				AF.r = math.Clamp( AF.r * 900,-turnrate,turnrate )
	...
	function ENT:Think()	
		...
		if self.Explode then
			...
			util.BlastDamage( IsValid( Inflictor ) and Inflictor or Entity(0), IsValid( Attacker ) and Attacker or Entity(0), self:GetPos(),450,150)
	...
	function ENT:Explosion( pos )
		...
		for i = 0,20 do
			...
			if particle then
				particle:SetVelocity( VectorRand(-1,1) * 400 )
				particle:SetDieTime( math.Rand(4,6) )
