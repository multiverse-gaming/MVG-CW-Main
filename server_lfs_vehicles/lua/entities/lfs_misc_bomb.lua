...
if SERVER then
	...
	function ENT:BlindFire()
		...
		if IsValid( pObj ) then
			pObj:SetVelocityInstantaneous( self:GetForward() * (self:GetStartVelocity() + 1500) )
		end
	end
	
	function ENT:FollowTarget( followent )
		local speed = self:GetStartVelocity() + (self:GetDirtyMissile() and 2000 or 1500)
		local turnrate = (self:GetCleanMissile() or self:GetDirtyMissile()) and 40 or 30
		...
		local pos = TargetPos + followent:GetVelocity() * .3
		...
		if IsValid( pObj ) then
			if not self:GetDisabled() then
				...
				AF.p = math.Clamp( AF.p * 500,-turnrate,turnrate )
				AF.y = math.Clamp( AF.y * 500,-turnrate,turnrate )
				AF.r = math.Clamp( AF.r * 500,-turnrate,turnrate )
	...
	function ENT:Think()	
		...
		if self.Explode then
			...
			util.BlastDamage( IsValid( Inflictor ) and Inflictor or Entity(0), IsValid( Attacker ) and Attacker or Entity(0), self:GetPos(),800,1500)
	...
	function ENT:OnRemove()
		...
		sound.Play( "gbombs_5/explosions/heavy_bomb/ex1.mp3", Pos, 95, 140, 1 )
