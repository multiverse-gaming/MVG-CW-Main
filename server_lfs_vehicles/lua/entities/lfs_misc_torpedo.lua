...
if SERVER then
	...
	function ENT:BlindFire()
		...
		if IsValid( pObj ) then
			pObj:SetVelocityInstantaneous( self:GetForward() * (self:GetStartVelocity() + 2000) )
		end
	end
	
	function ENT:FollowTarget( followent )
		local speed = self:GetStartVelocity() + (self:GetDirtyMissile() and 2500 or 3000)
		local turnrate = (self:GetCleanMissile() or self:GetDirtyMissile()) and 50 or 40
		...
		local pos = TargetPos + followent:GetVelocity() * 0.35
		...
		if IsValid( pObj ) then
			if not self:GetDisabled() then
				...
				AF.p = math.Clamp( AF.p * 700,-turnrate,turnrate )
				AF.y = math.Clamp( AF.y * 700,-turnrate,turnrate )
				AF.r = math.Clamp( AF.r * 700,-turnrate,turnrate )
	...
	function ENT:Think()	
		...
		if self.Explode then
			...
			util.BlastDamage( IsValid( Inflictor ) and Inflictor or Entity(0), IsValid( Attacker ) and Attacker or Entity(0), self:GetPos(),200,750)
