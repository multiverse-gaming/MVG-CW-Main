...
--self.flightvector = self.Entity:GetForward() * 180
self.flightvector = nil
self.timeleft = CurTime() + 40

self.AirburstTime = CurTime() + 40
self.Entity:SetModel( "models/dav0r/hoverball.mdl" )
self.Entity:PhysicsInit( SOLID_VPHYSICS )      -- Make us work with physics,  	
self.Entity:SetMoveType( MOVETYPE_NONE )   --after all, gmod is a physics  	
self.Entity:SetSolid( SOLID_VPHYSICS )        -- CHEESECAKE!    >:3           
self.Entity:SetColor(30,144,255,0)

Tracer = ents.Create("env_spritetrail")
Tracer:SetKeyValue("lifetime","0.3")
Tracer:SetKeyValue("startwidth","200")
Tracer:SetKeyValue("endwidth","0")
Tracer:SetKeyValue("spritename","trails/plasma.vmt")
Tracer:SetKeyValue("rendermode","5")
Tracer:SetKeyValue("rendercolor","30 144 255")
...
Glow:SetKeyValue("model","orangecore1.vmt")
Glow:SetKeyValue("rendercolor","30 144 255")
...
Glow:Activate()
 ...
 function ENT:Think()
		...
		self.Entity:Remove()				
			...
			util.BlastDamage(self.Entity, self.Entity, self.Entity:GetPos(), 700, 1000)
			local effectdata = EffectData()
			effectdata:SetOrigin(self.Entity:GetPos())
			effectdata:SetScale(2.2)
	...
	local tr = util.TraceLine( trace )
				...
				if tr.Hit then
					...
					effectdata:SetScale( 1000 )			// Big splash for big bullets
					util.Effect( "watersplash", effectdata )
					self.Entity:Remove()
					return true
					end

					util.BlastDamage(self.Entity, self.Entity, tr.HitPos, 400, 1000)
					local effectdata = EffectData()
