...
function ENT:Initialize()
	self:SetModel( "models/lordtrilobits/starwars/props/kyber_crate_phys.mdl" )
	...
	local explode = ents.Create( "env_explosion" )
		...
		explode:SetKeyValue( "iRadiusOverride", 100000 )
...
function ENT:Use( activator, caller )
    ...
    util.BlastDamage(self, activator, self:GetPos(), 500, 10000)
    local explode = ents.Create( "env_explosion" )
    explode:SetPos( self:GetPos() )
    explode:Spawn()
    explode:SetKeyValue( "spawnflags", 144 )
	...
	prop:SetModel("models/lordtrilobits/starwars/props/kyber_crate_phys.mdl")
