...
function ENT:Use(activator, caller)
	...
	if self.UseTimer <= CurTime() and activator:IsPlayer() and activator:GetMaxHealth() > activator:Health() then
		
		self:SetSkin(self.LoadingSkin)
		self:EmitSound("buttons/button6.wav")

		activator:SetHealth( math.min( activator:GetMaxHealth(), activator:Health() + heal_amount ) )
