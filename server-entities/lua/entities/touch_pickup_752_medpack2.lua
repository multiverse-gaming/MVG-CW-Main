...
function ENT:Initialize()
	self.Entity:SetModel("models/starwars/items/shield_large.mdl")
...
function ENT:Use( activator, caller )
	if (activator:IsPlayer()) then
		...
		if (activator:Armor() >= activator:GetMaxArmor()) then -- If overhealed dont do anything
			activator:PrintMessage( HUD_PRINTTALK, "[Medical Support] No effect, used 25 AP Packet")
		elseif (activator:Armor() + armor > activator:GetMaxArmor()) then -- If will overheal set MAX HP.
			...
			activator:PrintMessage( HUD_PRINTTALK, "[Medical Support] Armoured PARTIALLY using 25 AP Packet")
		else
			activator:SetArmor(activator:Armor()+armor)
			activator:PrintMessage( HUD_PRINTTALK, "[Medical Support] Armoured using 25 AP Packet")
		end
		self.Entity:Remove()
