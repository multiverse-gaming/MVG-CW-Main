...
net.Receive("deceive.interface", function(_, ply)
	...
	if deceive.Config and deceive.Config.NoDisguiseAsJobs and table.HasValue(deceive.Config.NoDisguiseAsJobs, ply:getJobTable(true).name) then
		deceive.Notify(ply, "disguise_disallowed_as", NOTIFY_ERROR, 5, { ply:getJobTable(true).name })
	...
	if IsValid(ply.Deceive_Using) and ply.Deceive_Using:GetPos():Distance(ply:GetPos()) < 95 then
		...
		if deceive.Config and deceive.Config.NoDisguiseIntoJobs and table.HasValue(deceive.Config.NoDisguiseIntoJobs, target:getJobTable(true).name) then
			deceive.Notify(ply, "disguise_disallowed_to", NOTIFY_ERROR, 5, { target:getJobTable(true).name })
