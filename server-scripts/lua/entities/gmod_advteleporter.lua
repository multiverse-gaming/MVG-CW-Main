	...
	function ENT:Think()
		...
		if IsValid(self:GetDestination()) then
			...
			for i,ent in pairs(area) do
				...
				if IsValid(ent) and ent:IsPlayer() and ent ~= self and ent ~= self:GetDestination()
				and not table.HasValue(self.tEnts,ent) and not table.HasValue(self:GetDestination().tEnts,ent) 
				and (self:GetTeleOnTouch() or self.KeyOn or self.WireTeleport or (self:GetTeleOnUse() and self.IsBeingUsed)) 
				and not self:GetDestination().Locked and self.NextTeleport < CurTime() 
		...
		if self.IsBeingUsed and IsValid(self.Activator) and self.Activator:IsPlayer()
		and (self.Activator:KeyReleased(IN_USE) or not self.Activator:GetEyeTraceNoCursor().Entity == self) then
			self.IsBeingUsed = false
		end
