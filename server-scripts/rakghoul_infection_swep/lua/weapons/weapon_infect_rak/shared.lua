...
SWEP.WerewolfModel	= "models/grealms/characters/rakghoul/rakghoul.mdl"
...
if ( CLIENT ) then
	SWEP.PrintName	= "Rakghoul Infection SWEP"	 
	SWEP.Category	= "Star Wars Rakghoul SWEP"
...
function SWEP:Deploy()
	if CLIENT then 
		chat.AddText(
		Color( 0, 200, 0 ), "Rakghoul Infection System (Controls):")
		chat.AddText(
...
function SWEP:RegeneratingHealth(owner)
	...
	self.HealId = "WEAPON_INFECT_RAK"..self.HealId..owner:Name()
...
function SWEP:Infection(dmg)
	...
	if SERVER then dmg:ChatPrint("You have contracted Rakghoul curse, you will succumb in "..(self.InfectDuration*5).." seconds.") end
...
function SWEP:Think()
	...
	if self.PlayerModel == nil then
		...
		self.WerewolfScale = self.PlayerScale * 1.15
