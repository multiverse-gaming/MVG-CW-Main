...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("maintaince_grid")
		ent:SetPos(SpawnPos)
	ent:Spawn()
	ent:Activate()
	...
	if Health > 1 then
		...
		if not self.Entity:IsOnFire() then
			...
			for k, v in pairs( player.GetAll() ) do
				v:PrintMessage( HUD_PRINTTALK, "[AI] A EPS Hatch is Damaged find it before a fire starts" )
			end
		end
	end
...
function ENT:Use(activator, caller)
	...
	if (activator:IsPlayer()) then
		if HealthonE == 0 then
		activator:PrintMessage( HUD_PRINTTALK, "[EPS Hatch] ERROR CODE 1" )
		else
		activator:PrintMessage( HUD_PRINTTALK, "[EPS Hatch] Running > Status: "..HealthonE.."%" )
		end
	end
end