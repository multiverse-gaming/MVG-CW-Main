...
ENT.Category		= "StarWars Turrets"
ENT.PrintName 		= "LAAT Lazer Cannon"
ENT.Author			= "Tac"
ENT.Spawnable		= true
...
function ENT:DoShot()
	...
	if self.LastShot + self.ShotInterval < CurTime() then
		if SERVER then
			...
			util.Effect( "lazer_grenade_laat", effectdata )
		...
		if IsValid(self.shootPos) and SERVER then
			local nade = ents.Create("laat_lazer")
			...
			nade.Entity:SetRenderMode( RENDERMODE_TRANSCOLOR )     
			
			nade.Entity:SetColor( Color(0, 0, 0, 0) )
