...
function ENT:Initialize()
	...
	self.Entity:SetModel("models/props/starwars/tech/mainframe.mdl")
	...
	self.Entity:SetNWInt( "maxhealth", 1000 )
	...
	if Health > 1 then
		...
		if not self.Entity:IsOnFire() then
			...
			for k, v in pairs( player.GetAll() ) do
				v:PrintMessage( HUD_PRINTTALK, "[AI] A Shieldgenerator has been destroyed!" )
			end
		end
	end
...
function ENT:Use(activator, caller)
	...
	if (activator:IsPlayer()) then
		if HealthonE == 0 then
		activator:PrintMessage( HUD_PRINTTALK, "[Shieldgenerator] CRITICAL ERROR" )
		else
		activator:PrintMessage( HUD_PRINTTALK, "[Shieldgenerator] Running > Shieldlevel: "..HealthonE.."%" )
		end
	end
end