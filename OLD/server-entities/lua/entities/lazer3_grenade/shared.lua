...
ENT.PrintName 		= "Triple Lazer Cannon Grenade"
ENT.Author			= "Tac"
ENT.Spawnable		= true
...
function ENT:DoShot()
	...
	if self.LastShot+self.ShotInterval<CurTime() then
		if SERVER then
			...
			util.Effect( "blue_tracer_fx", effectdata )
		...
		if IsValid(self.shootPos) and SERVER then
			...
			local nade = ents.Create("lazer3_g")
			...
			nade.flightvector = self:GetRight() *100
