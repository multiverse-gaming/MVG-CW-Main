...
function ENT:Use( activator, caller )
	if (activator:IsPlayer()) then
		...
		if (activator:Armor() >= activator:GetMaxArmor()) then -- If overhealed dont do anything
			activator:PrintMessage( HUD_PRINTTALK, "[Medical Support] No effect, used 50 AP Packet")
		elseif (activator:Armor() + armor > activator:GetMaxArmor()) then -- If will overheal set MAX HP.
			...
			activator:PrintMessage( HUD_PRINTTALK, "[Medical Support] Armoured PARTIALLY using 50 AP Packet")
		else
			activator:SetArmor(activator:Armor()+armor)
			activator:PrintMessage( HUD_PRINTTALK, "[Medical Support] Armoured using 50 AP Packet")
		end
		self.Entity:Remove()
