...
function ENT:Use( activator, caller )
	if (activator:IsPlayer()) then
		...
		if (activator:Health() >= activator:GetMaxHealth()) then -- If overhealed dont do anything
			activator:PrintMessage( HUD_PRINTTALK, "[Medical Support] No effect, used 100 HP Bacta")

		elseif (activator:Health() + heal > activator:GetMaxHealth()) then -- If will overheal set MAX HP.
			activator:SetHealth(activator:GetMaxHealth())
			activator:PrintMessage( HUD_PRINTTALK, "[Medical Support] Healed PARTIALLY using 100 HP Bacta")
		else
			activator:SetHealth(activator:Health()+heal)
			activator:PrintMessage( HUD_PRINTTALK, "[Medical Support] Healed using 100 HP Bacta")
		end
		self.Entity:Remove()
