...
local ent = ents.Create( "ahsoka_vs_maul" )
ent:SetPos( SpawnPos )
ent:Spawn()
ent:Activate()
return ent
...
self.Entity:EmitSound("mvg_music/ahsoka_vs_maul.mp3",600)

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Ahsoka vs Maul activated")
end
...
self.Entity:StopSound("mvg_music/ahsoka_vs_maul.mp3")

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Ahsoka vs Maul deactivated")
...
for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Ahsoka vs Maul removed")
ply:SendLua([[RunConsoleCommand("stopsound")]])
end
