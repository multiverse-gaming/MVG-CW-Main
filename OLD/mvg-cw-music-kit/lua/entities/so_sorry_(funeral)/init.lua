...
local ent = ents.Create( "so_sorry_(funeral)" )
ent:SetPos( SpawnPos )
ent:Spawn()
ent:Activate()
return ent
...
self.Entity:EmitSound("Oogway/so_sorry_(funeral).mp3",600)

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "So sorry Funeral activated")
end
...
self.Entity:StopSound("Oogway/so_sorry_(funeral).mp3")

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "So sorry Funeral deactivated")
...
for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "So sorry Funeral removed")
ply:SendLua([[RunConsoleCommand("stopsound")]])
end
