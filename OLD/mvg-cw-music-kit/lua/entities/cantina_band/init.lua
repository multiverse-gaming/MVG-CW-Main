...
function ENT:Initialize()
	...
	self.Entity:SetNetworkedString( 98, "" .. sirensquadnames[sirensquadnumber] .. "" )
...
local ent = ents.Create( "cantina_band" )
ent:SetPos( SpawnPos )
ent:Spawn()
ent:Activate()
return ent
...
local squad = self:GetNetworkedString( 98 )

self.Entity:EmitSound("Oogway/cantina_band.mp3",600)

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Cantina Band activated")
end
...
self.Entity:StopSound("Oogway/cantina_band.mp3")

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Cantina Band deactivated")
...
for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Cantina Band removed")
ply:SendLua([[RunConsoleCommand("stopsound")]])
end
