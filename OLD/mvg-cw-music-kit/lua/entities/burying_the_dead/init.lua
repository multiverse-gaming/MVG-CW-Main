...
function ENT:Initialize()
	...
	self.Entity:SetNetworkedString( 96, "" .. sirensquadnames[sirensquadnumber] .. "" )
...
local ent = ents.Create( "Burying_the_Dead" )
ent:SetPos( SpawnPos )
ent:Spawn()
ent:Activate()
return ent
...
local squad = self:GetNetworkedString( 96 )

self.Entity:EmitSound("Oogway/Burying_the_Dead.mp3",600)

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Burying the Dead activated")
end
...
self.Entity:StopSound("Oogway/Burying_the_Dead.mp3")

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Burying the Dead deactivated")
...
for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "Burying the Dead removed")
ply:SendLua([[RunConsoleCommand("stopsound")]])
end
