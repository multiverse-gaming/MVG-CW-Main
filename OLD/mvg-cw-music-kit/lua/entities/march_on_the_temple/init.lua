...
function ENT:Initialize()
	...
	self.Entity:SetNetworkedString( 16, "" .. sirensquadnames[sirensquadnumber] .. "" )
...
local ent = ents.Create( "march_on_the_temple" )
ent:SetPos( SpawnPos )
ent:Spawn()
ent:Activate()
return ent
...
local squad = self:GetNetworkedString( 16 )

self.Entity:EmitSound("Oogway/march_on_the_temple.mp3",600)

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "March on the Temple activated")
end
...
self.Entity:StopSound("Oogway/march_on_the_temple.mp3")

for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "March on the Temple deactivated")
...
for k,ply in pairs(player.GetAll()) do
ply:ChatPrint( "March on the Temple removed")
ply:SendLua([[RunConsoleCommand("stopsound")]])
end
