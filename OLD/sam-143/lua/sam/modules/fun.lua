...
command.new("heal")
	...
	:OnExecute(function(ply, targets)
		...
		for i = 1, #targets do
			local maxhealth = targets[i]:GetMaxHealth()
			targets[i]:SetHealth(maxhealth)
		end

		sam.player.send_message(nil, "heal", {
			A = ply, T = targets
		})
	end)
	...
	:OnExecute(function(ply, targets, model)
		...
		if not util.IsValidModel(model) then
			...
			for i = 1, #targets do
				...
				targets[i]:SetNWString("EventModel", nil)
...
command.new("unmodel")
	...
	:SetPermission("unmodel", "superadmin")
	...
	:OnExecute(function(ply, targets, model)
		...
		for i = 1, #targets do
			...
			gamemode.Call("PlayerSetModel", targets[i])
		...
		sam.player.send_message(nil, "unmodel", {
