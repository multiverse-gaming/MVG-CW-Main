...
if SERVER then
	local maps = {}
	local current = 1
	local blacklistedMaps = {"test_hardware","test_speakers","cs_assault","cs_compound","cs_havana","cs_italy","cs_militia","cs_office","de_cbble","de_chateau","de_dust","de_dust2","de_inferno","de_nuke","de_piranesi","de_port","de_prodigy","de_tides","de_train","de_aztec"}
	for k, v in ipairs(file.Find("maps/*.bsp", "GAME")) do
		isBlacklisted = false
		local currentMap = v:sub(1, -5):lower()
