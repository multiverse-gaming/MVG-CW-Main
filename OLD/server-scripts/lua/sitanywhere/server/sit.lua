...
local SittingOnPlayer = false
local SittingOnPlayer2 = false
local PlayerDamageOnSeats = true
local AllowWeaponsInSeat = false
local AdminOnly = false
local FixLegBug = true
local AntiPropSurf = true
local AntiToolAbuse = true
local AllowSittingTightPlaces = false
...
local function Sit(ply, pos, ang, parent, parentbone,  func, exit)
	...
	elseif AllowWeaponsInSeat then
...
function META.Sit(ply, EyeTrace, ang, parent, parentbone, func, exit, wantedAng)
	...
	if SittingOnPlayer then -- Sitting on SITTING Players
	...
	if IsValid( EyeTrace.Entity ) and EyeTrace.Entity:IsPlayer() and SittingOnPlayer2 and shouldSitOnPlayer then
		ent = EyeTrace.Entity
		...
		if ang_smallest_hori and distsang[infront].Hit and distsang[infront].Distance > 14 and smallest_hori <= 16 then
			...
			if not EyeTrace.HitWorld then
				...
				if ent:IsPlayer() then
					if not SittingOnPlayer2 then return end
...
local function checkAllowSit(ply)
	...
	if not AllowSittingTightPlaces and trace.HitWorld then return false end
	...
	if AdminOnly and not ply:IsAdmin() then
		return false
	end
...
for _,v in next, PickupAllowed do
	hook.Add(v, "SA_DontTouchYourself", function(ply, ent)
		if AntiPropSurf and CheckSeat2(ply, ent) == false then
			return false
		end
	end)
end
...
hook.Add("CanTool", "SA_DontTouchYourself", function(ply, tr)
	if AntiToolAbuse and IsValid(tr.Entity) and CheckSeat2(ply, tr.Entity) == false then
		return false
	end
end)
...
hook.Add("EntityRemoved","Sitting_EntityRemoved",function(ent)
	if FixLegBug and ent.playerdynseat and IsValid(ent.sittingPly) then
