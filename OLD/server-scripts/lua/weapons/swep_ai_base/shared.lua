...
SWEP.Primary.Spread				= 0.2 --How inaccurate the weapon should be. Examples: AWP - 0.003, M4A1 - 0.030, MAC10 - 0.060. Spread acts differently for projectile weapons, you need a higher value to get the same amount of spread.
SWEP.Primary.SpreadMoveMult		= 10 --How much should we multiply the spread if the NPC is moving? Higher values mean the weapon is more inaccurate while moving. Rule of thumb is 1.2 for rifles, 1.1 for pistols, 1 for SMGs, 1.3-1.5 for MGs, and 5+ for sniper rifles.
...
function SWEP:Initialize()
    ...
    local Profiency = self.Profiency 


    
    local owner = self:GetOwner()
    if owner:IsNPC() and SERVER then
        timer.Simple(0, function()
            self.Proficiency = owner:GetCurrentWeaponProficiency()

            local currentWep = self.Primary.Spread

            Profiency = self.Proficiency
            if Profiency >= 4 then
                self.Spread = currentWep * 0.5
            elseif Profiency == 3 then
                self.Spread = currentWep
            elseif Profiency == 2 then
                self.Spread = currentWep * 1.5
            elseif Profiency == 1 then
                self.Spread = currentWep * 2.5
            elseif Profiency == 0 then
                self.Spread = currentWep * 3.5
            end
...
function SWEP:Shoot(forceTargetPos) --forceTargetPos is used to force NPCs to shoot at air if their target is already dead but the gun is configured to fire full bursts without stopping
    ...
    if self.Primary.Type == "bullet" then
        ...
        local DamageMultiplier = self.Owner.DamageMultiplier
        if (DamageMultiplier) != nil and DamageMultiplier >= 0 then
            bulletInfo.Damage = self.Primary.Damage * DamageMultiplier
        else
            bulletInfo.Damage = self.Primary.Damage  * 0.2
        end
