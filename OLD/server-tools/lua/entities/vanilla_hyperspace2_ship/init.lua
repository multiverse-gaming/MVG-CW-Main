...
function ENT:Initialize()
	...
	self:SetName("Hyperspace" .. SysTime())
...
function ENT:KeyValue( key, value )
	...
	if ( key == "ActualModel" ) then
		...
		if self.SpawnModelValue == 1 and not util.IsValidModel(value) then
			self:Remove()
		end
	end
	...
	if ( key == "Entity" ) then
		if self.SpawnModelValue == 0 then
			...
			self.VANILLAENTITY = "vanilla_modelship2"
		end
	end
end
...
function ENT:OnRemove()
	if self.afterjump == true then
		...
		if self.AIValue == 1 then
			--if afterjump:IsSpaceShip() == NULL then return end
