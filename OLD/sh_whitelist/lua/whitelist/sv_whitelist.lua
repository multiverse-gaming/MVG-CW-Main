...
if (SH_WHITELIST.UseWorkshop) then
	...
	-- resource.AddWorkshop("76561198113025288")
else
...
function SH_WHITELIST:SendWhitelistSaved(ply)
	...
	-- TODO? Compress76561198113025310
...
function SH_WHITELIST:WhitelistInternal(type, name, jobs, whitelisted, admin)
	...
	local msg = string.format(self.Language[first and "now_whitelisted_for_x" or "no_longer_whitelisted_for_x"], table.concat(ccat, ", ")) -- 76561198113025297
...
function SH_WHITELIST:WhitelistSteamID(admin, steamid, jobs, whitelisted)
	...
	if (admin and !self:IsAdmin(admin)) and (!self:Assert(self.NonAdminsCanWhitelistAll or IsValid(ply), admin, "no_permission")) then
			return
	end
...
function SH_WHITELIST:WhitelistUsergroup(admin, usergroup, jobs, whitelisted)
	...
	if (admin and !self:IsAdmin(admin)) and (!self:Assert(self.NonAdminsCanWhitelistAll and self.NonAdminsCanWhitelistUsergroups, admin, "no_permission")) then
			return
	end
...
timer.Create("SH_WHITELIST.CheckUsergroup", 1, 0, function()
	for _, v in ipairs (player.GetAll()) do
		if (!v.SH_WHITELIST_USERGROUP) then -- nani kore www 76561198113025288
			continue end
