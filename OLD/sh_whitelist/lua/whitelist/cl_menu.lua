-- IDEA: Presets and 76561198113025297
...
function SH_WHITELIST:OpenMenu(opento)
	...
	_SH_WHITELIST = wnd
		...
		jobswl:Dock(FILL)
			...
			a:SetMinWidth(64)
			a:SetMaxWidth(64)
			ilist:SetPaintBackground(false)
			ilist:Dock(FILL)
			ilist.OnRowRightClick = function(me, _, __)
				...
				local men = self:Menu()
				men:AddOption(L("toggle_selected", #sel), function()
					local b = false
					local chkd = 0
			...
			local function AddJob(tx, id, color)
				...
				-- Checked?
				local whitelisted = (selusg and self.Whitelisted.usergroups[selusg] or self.Whitelisted.players[selid]) or {} /* 76561198113025309 */
				local b = whitelisted[tx] == true

				local chkw = vgui.Create("DPanel", ilist)
				chkw:SetPaintBackground(false)
