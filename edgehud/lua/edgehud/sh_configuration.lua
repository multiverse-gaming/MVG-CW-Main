...
if SERVER then
	...
	net.Receive("EdgeHUD:RequestConfig", function( _, ply )
		...
		if ply.EdgeHUDConfig and ply.EdgeHUDConfig > CurTime() then return end
	...
	if table.Count(EdgeHUD.Configuration.Config) > 0 then
		...
		timer.Create("EdgeHUD:ConfigRequester", 0, 0, function(  )
			timer.Adjust("EdgeHUD:ConfigRequester", 3)
			if table.Count(EdgeHUD.Configuration.Config) == 0 then
	...
	net.Receive("EdgeHUD:UpdatePlayerConfig",function(  )
		...
		timer.Remove("EdgeHUD:ConfigRequester")
