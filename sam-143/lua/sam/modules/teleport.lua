...
command.new("tp")
	...
	:OnExecute(function(ply, targets)
		...
		for i = 1, #targets do
			...
			target:ExitVehicle()
			target:SetVelocity(Vector(0, 0, 0))
			target:SetPos(find_empty_pos(tr.HitPos, target))
			target:SetEyeAngles((ply:EyePos() - target:EyePos()):Angle())
...
command.new("send")
	...
	:OnExecute(function(ply, targets, targettings)
		local target = targets[1]
		local targetting = targettings[1]
		...
		if targetting:sam_get_exclusive(ply) then
			ply:sam_send_message(targetting:sam_get_exclusive(ply))
		end
		...
		target:ExitVehicle()
		target:SetVelocity(Vector(0, 0, 0))
		target:SetPos(find_empty_pos(targetting:GetPos(), target))
		target:SetEyeAngles((target:EyePos() - targetting:EyePos()):Angle())
