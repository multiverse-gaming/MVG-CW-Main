...
command.new("clearpac")
		:SetPermission("clearpac", "admin")
		...
		:OnExecute(function(ply, targets)
			...
			for i = 1, #targets do
				...
				target:SendLua([[RunConsoleCommand("pac_clear_parts")]])
			end

			if sam.is_command_silent then return end
			sam.player.send_message(nil, "clearpac", {
				A = ply, T = targets
			})
		end)
	:End()
	...
	:OnExecute(function(ply)
		...
		for i, v in ipairs( player.GetAll() ) do
			if v:HasPermission("see_admin_chat") then
				v:sam_send_message("[STAFF] {S Red} has entered admin mode.", {S = ply:GetName()} )
			end
		end
...
command.new("unadmin")
	...
	:OnExecute(function(ply)
		...
		for i, v in ipairs( player.GetAll() ) do
			if v:HasPermission("see_admin_chat") then
				v:sam_send_message("[STAFF] {S Red} has exited admin mode.", {S = ply:GetName()} )
			end
		end
...
command.new("givetac")
	:SetPermission("givetac", "superadmin")
	...
	:OnExecute(function(ply, targets, weapon)
		for i = 1, #targets do
			targets[i]:Give("seal6-tac-insert")
		end

		sam.player.send_message(nil, "givetac", {
			A = ply, T = targets, V = weapon
		})
	end)
:End()
...
command.new("givereg")
	:SetPermission("givereg", "superadmin")
	...
	:OnExecute(function(ply, targetName, weapon)
		

	local currentCategories_Jobs = DarkRP.getCategories()["jobs"] 
	local numLooped = 0

	local CategoryNames = nil

	local curCategory = {}

	do -- To get all possible category names
		for i, v in pairs (currentCategories_Jobs) do

			CategoryNames = CategoryNames or {}

			table.insert(CategoryNames, #CategoryNames, v.name)
		end
	end

	for i, v in pairs (CategoryNames) do
		local curFind = string.find(v, targetName)

		if curFind != nil then
			numLooped = numLooped + 1
			print(v)

			table.insert(curCategory, #curCategory, v)
		end
	end
	...
	if numLooped > 1 then
		...
		local amount = #curCategory

		local indexC = 0
		for i, v in pairs (curCategory) do

			print(v)
			print(indexC)
			if indexC == amount then
		...
		sam.player.send_message(ply, "givereg_toomany", {A = concatString})

		return
	elseif numLooped == 1 then
		curCategory = curCategory[0]

	else
		sam.player.send_message(nil, "givereg_nofound", { })
	...
	local teamsToTarget = nil

	for i, v in pairs (currentCategories_Jobs) do -- Category section
		if v.name == curCategory then
			for k,t in pairs (v) do -- Get all members in job
				if k == "members" then 
					for b1, b2 in pairs (t) do
						teamsToTarget = teamsToTarget or {}
		
						table.insert(teamsToTarget, #teamsToTarget, b2.team)
					end
				end
			end
		end
	...
	for i, v in pairs (teamsToTarget) do
		local curTeamPlayers = team.GetPlayers(v)

		for i, v in pairs (curTeamPlayers) do
			v:Give(weapon)
		end
	end
	...
	sam.player.send_message(nil, "givereg", {
		A = ply, T = curCategory, V = weapon
	})
end)
:End()
