...
function ENT:Draw()
    ...
    cam.Start3D2D(self:LocalToWorld(Vector(0, CENTER.y, self:OBBMaxs().z)) + Vector(0, 0, 10), ang, 0.1)
        ...
        draw.RoundedBox(0, -( w / 2), h - (h / 12), w, (h / 12), RDV.LIBRARY.GetConfigOption("COMMS_overheadColor"))

        w, h = draw.SimpleText(RDV.LIBRARY.GetLang(nil, "COMMS_overheadName"), "RD_FONTS_CORE_OVERHEAD", 0, 0, COL_WHITE, TEXT_ALIGN_CENTER)
