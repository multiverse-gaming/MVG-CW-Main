...
function base_table:PaintScroll(panel)
	...
	scr.Paint = function(_, w, h)
		draw.RoundedBox(4, 0, 0, w, h, /* 76561198113025297 styl.header */ styl.bg)
	end
...
function base_table:StringRequest(title, text, callback)
	...
	cancel:SetPaintBackground(false)
...
-- https://facepunch.com/showthread.php?t=1522945&p=50524545&viewfull=1#post50524545|76561198113025297
...
function base_table:Notify(msg, dur, bg, parent)
	...
	local p = vgui.Create("DButton", IsValid(parent) and parent or nil)
	p:MoveToFront()
	...
	navbar.Paint = function(me, w, h)
		...
		for i, page in ipairs (tabs) do
			...
			page.m_Button = btn
				...
				local lbl = self:QuickLabel(tx, font_prefix .. "Medium", self.Style.text, btn)
				lbl:Dock(LEFT)
		...
		chk.Paint = function(me, w, h)
			if (me:GetChecked()) then
				if (me.m_bDrawBackground) then
					draw.RoundedBox(4, 0, 0, w, h, self.Style.header)
				end
...
hook.Add("InitPostEntity", font_prefix .. "ToolTipOverride", function()
	...
	function PANEL:PositionTooltip()
		if (self.m_bLounge) then
			...
			local tw = self.TargetPanel:GetSize()
	...
	function PANEL:Paint(w, h)
		if (self.m_bLounge) then
			...
			self:PositionTooltip()
			self:SetFont(font_prefix .. "Medium")
			self:SetTextColor(styl.text)

			draw.RoundedBox(4, 0, 0, w, h, self.m_bLoungeTooltipBG or styl.header)
