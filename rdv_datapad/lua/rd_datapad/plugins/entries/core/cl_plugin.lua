...
net.Receive("NS_DATAPAD_ArchiveConsole", function()
    ...
    FRAME.PaintOver = function(s, w, h)
        draw.SimpleText("Page "..tostring(PAGE), "RDV_DAP_FRAME_TITLE", w * 0.95, h * 0.1, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
    end
    ...
    SCROLL.PaintOver = function(s, w, h)
        if !foundData then
            draw.SimpleText(NCS_DATAPAD.GetLang(nil, "DAP_noEntriesSaved"), "RDV_DAP_FRAME_TITLE", w * 0.5, h * 0.4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    end 
    ...
    local function RetrievePage(newPage, FAILCB)
        newPage = math.Clamp(newPage, 0, 255)

        net.Start("NS_DATAPAD_GetArchived")
            net.WriteUInt(newPage, 8)
        ...
        net.Receive("NS_DATAPAD_GetArchived", function()
            ...
            if !SUCCESS and FAILCB then FAILCB(true, newPage) return end
            ...
            for k, v in ipairs(TAB) do
                ...
                if !foundData then foundData = true end
                ...
                but.DoRightClick = function(s)        
                    ...
                    MenuButtonOptions:AddOption(NCS_DATAPAD.GetLang(nil, "DAP_deleteEntryLabel"), function()
                        ...
                        RetrievePage(newPage, function(FAIL)
                            if FAIL then                
                                RetrievePage((FAIL - 1))
                            end
                        end )
    ...
    NEXT.DoClick = function()
        ...
        RetrievePage((PAGE + 1), function(FAIL)
            if FAIL then
                PAGE = oPage
            end
        end )
