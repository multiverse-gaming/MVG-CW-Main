...
function SWEP:PrimaryAttack()
	--[[
	if timer.Exists("to_check_tfa_grenade_allowed_to_throw_" .. self.Owner:EntIndex()) then
		self:SetNWBool("NextGrenadeReadyToPull", false)
		
	else
		self:SetNWBool("NextGrenadeReadyToPull", false)
		timer.Create("to_check_tfa_grenade_allowed_to_throw_" .. self.Owner:EntIndex(), self.RPMGrenadeBase, 1, function()
			self:SetNWBool("NextGrenadeReadyToPull", true)
		end)
	end

	if self:GetNWBool("NextGrenadeReadyToPull", false) == false then return end

	]]
    if not IsFirstTimePredicted() then return end
	if self:Clip1() > 0 and self:OwnerIsValid() and self:CanStartThrow() and self.CookingEnabled and self.Primary.Round == "rw_sw_ent_nade_thermal" then
...
function SWEP:SecondaryAttack()
	--[[
	print(self:GetNWBool("NextGrenadeReadyToPull", false) )
	if self:GetNWBool("NextGrenadeReadyToPull", false) == false then return end
	]]
...
function SWEP:SelfDestruct()
	...
	util.BlastDamage( self, self.Owner, self:GetPos(), 375, 350 )
	self:SetClip1( 0 )
	self:Deploy()
end
