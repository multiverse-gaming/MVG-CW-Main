att.PrintName = "DC-15a - Tibanna Overpressure"
att.Free = true
att.HideIfUnavailable = true

att.SortOrder = 20
att.Icon = Material("interfaz/armas/sw_powercore.png", "smooth mips")
att.Description = "Higher pressure compression allows for a more lethal blow"
att.Desc_Pros = {
}
att.Desc_Cons = {
}
att.Desc_Neutrals = {
}
att.Slot = "ultimatesniper"

att.AutoStats = true

att.Silencer = true

att.Mult_Damage = 7.14285714286 -- 250 Damage
att.Mult_DamageMin = 7.14285714286
att.Mult_RPM = 0.1333333333333333
att.Override_ClipSize = 4

att.Mult_AccuracyMOA = 0.00175438596
att.Mult_HipDispersion = 2

att.Mult_RecoilSide = 1.25
att.Mult_Recoil = 1.2

--Archived - Painite
--att.Mult_ShootVol = 1.25
--att.Mult_ShootPitch = 0.9 please don't

--att.Mult_MalfunctionMean = 0.7
-- att.Mult_PhysBulletMuzzleVelocity = 1.25

--att.Mult_RangeMin = 1
--att.Mult_Range = 550000