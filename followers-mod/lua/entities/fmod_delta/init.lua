...
local ent = ents.Create( "fmod_delta" )
...
self.npc:SetNWBool("FMOD_Delta", true)
self.npc:SetNWBool("FMOD_Special", true)
self.npc:SetNWBool("FMOD_Mortal", true)
self.npc:SetNWString("CustomName", "Delta")
...
self.npc:SetModel("models/humans/male/male_01.mdl")
...
self.npc.custommodel:SetModel("models/ddok1994_new_phys/1980_hazmat.mdl")
...
self.npc.light1.A = 0
self.npc.light1.B = -1.2
self.npc.light1.C = 3.5
self.npc.light1.AA = 0
self.npc.light1.BB = 0
self.npc.light1.CC = 0
self.npc.light1.AR = 255
self.npc.light1.BG = 255
self.npc.light1.CB = 255
self.npc.light1.FovA = 50
self.npc.light1.DisA = 650
self.npc.light1.FovB = 12
self.npc.light1.DisB = 30
self.npc.light1.Model = "models/props_wasteland/light_spotlight01_lamp.mdl"
self.npc.light1.ModelS = 0.12
self.npc.light1.ModelP = -1
...
self.npc.light1:Fire("setparentattachment", self.npc:GetAttachments()[1].name)
self.npc.light1:Spawn()
self.npc.light1:Activate()
self.npc.light1.cm:SetSkin(1)
self.npc:SetNWEntity("FMOD_Light", self.npc.light1)
self:DeleteOnRemove(self.npc.light1)
...
local act = npc:GetActivity()
if (!npc:GetNWBool("FMOD_InVehicle")) and npc:GetNWBool("FMOD_SpecialDown") and (!npc:GetNWBool("FMOD_DeltaDownAnim")) then
...
npc:EmitSound("FMod_Down.Delta")
npc:SetNWBool("FMOD_DeltaDownAnim", true)
...
npc:SetNWBool("FMOD_DeltaDownAnim", false)
npc:EmitSound("FMod_Wake.Delta")
...
if IsValid(npc) and IsValid(npc.light1) then
local enemy = npc:GetEnemy()
local anim = npc:GetSequenceName(self.npc:GetSequence())
local act = npc:GetActivity()
if npc:GetNWBool("FMOD_InVehicle") and (!npc.light1:GetNWBool("FMOD_LightInvisible")) then
npc.light1:SetNWBool("FMOD_LightInvisible",true)
npc.light1.cm:SetNoDraw(true)
elseif (!npc:GetNWBool("FMOD_InVehicle")) and (npc.light1:GetNWBool("FMOD_LightInvisible")) then
npc.light1:SetNWBool("FMOD_LightInvisible",false)
npc.light1.cm:SetNoDraw(false)
end
if IsValid(npc:GetNWEntity("FMOD_MyTarget")) and npc:GetNWEntity("FMOD_MyTarget"):FlashlightIsOn() and (!npc:GetNWBool("FMOD_InVehicle")) and (!npc:GetNWBool("FMOD_Delta_LightOn")) then
npc.light1.fa:Fire("TurnOn", "", 0)
npc.light1.fb:Fire("LightOn", "", 0)
npc.light1.cm:SetSkin(0)
npc:SetNWBool("FMOD_Delta_LightOn", true)
npc:EmitSound("FMod_Special.LightOn")
elseif ((!IsValid(npc:GetNWEntity("FMOD_MyTarget"))) or (npc:GetNWBool("FMOD_InVehicle")) or (IsValid(npc:GetNWEntity("FMOD_MyTarget")) and !npc:GetNWEntity("FMOD_MyTarget"):FlashlightIsOn())) and (npc:GetNWBool("FMOD_Delta_LightOn")) then
npc.light1.fa:Fire("TurnOff", "", 0)
npc.light1.fb:Fire("LightOff", "", 0)
npc.light1.cm:SetSkin(1)
npc:SetNWBool("FMOD_Delta_LightOn", false)
...
function ENT:MedkitThrow(npc)
if IsValid(npc) and (!npc:GetNWBool("FMOD_InVehicle")) then
if IsValid(npc:GetNWEntity("FMOD_MyTarget")) and npc:Visible(npc:GetNWEntity("FMOD_MyTarget")) and npc:GetNWEntity("FMOD_MyTarget"):Health() <= npc:GetNWEntity("FMOD_MyTarget"):GetMaxHealth()*0.8 and (npc:GetPos():Distance(npc:GetNWEntity("FMOD_MyTarget"):GetPos())<=npc:GetNWFloat("FMOD_MaxThrowDist")) then
if (npc:GetNWFloat("FMOD_MedkitThrow") <= CurTime()) and (!npc:GetNWBool("ActSequencePlay")) and (!npc:GetNWBool("FMOD_SpecialDown")) then
self:ActNPC(npc, "throw1", false, true, nil, nil, nil, nil, npc:GetNWEntity("FMOD_MyTarget"), "FMod_Medkit.Delta", nil, nil)
timer.Simple(0.8, function()
if (!IsValid(npc)) or (!IsValid(npc:GetNWEntity("FMOD_MyTarget"))) then return end
local shoot_angle = Vector(0,0,0)
local Dist=(npc:GetNWEntity("FMOD_MyTarget"):GetPos()-npc:GetPos()):Length()
shoot_angle = npc:GetNWEntity("FMOD_MyTarget"):GetPos()-npc:GetPos() 
shoot_angle = shoot_angle + Vector(math.random(-35,35),math.random(-35,35),math.random(-35,35))*(shoot_angle:Distance(Vector(0,0,0))/math.random(1800,1000))
shoot_angle:Normalize()
local shoot_pos = npc:GetPos() + npc:GetRight() * 5 + npc:GetUp() * 50 + shoot_angle * 55

local bone = npc:LookupBone("ValveBiped.Bip01_R_Hand")
local pos, ang = npc:GetBonePosition(bone)
if bone then
npc.medkit = ents.Create("item_healthvial")
if (IsValid(npc.medkit)) then
npc.medkit:SetPos(pos+ang:Right()+ang:Forward()+ang:Up())
npc.medkit:SetAngles(shoot_angle:Angle())
npc.medkit:SetOwner(npc)
npc.medkit:Spawn()
SafeRemoveEntityDelayed(npc.medkit, 5)
local trashphys = npc.medkit:GetPhysicsObject()
if IsValid(trashphys) then
trashphys:SetMass(5)
if Dist <= npc:GetNWFloat("FMOD_MaxThrowDist") then
trashphys:SetVelocity((shoot_angle*Dist)+(shoot_angle*Dist)*math.random(1,1.5))
trashphys:AddAngleVelocity(Vector(math.random(-800,800),math.random(-800,800),math.random(-800,800)))
end
if Dist > npc:GetNWFloat("FMOD_MaxThrowDist") then
trashphys:SetVelocity((shoot_angle*npc:GetNWFloat("FMOD_MaxThrowDist"))+(shoot_angle*npc:GetNWFloat("FMOD_MaxThrowDist"))*math.random(1,1.5))
trashphys:AddAngleVelocity(Vector(math.random(-800,800),math.random(-800,800),math.random(-800,800)))
end
end
end
...
npc:SetNWFloat("FMOD_MedkitThrow", CurTime() + npc:GetNWFloat("FMOD_MedkitThrow_"))
end
end
end
end
