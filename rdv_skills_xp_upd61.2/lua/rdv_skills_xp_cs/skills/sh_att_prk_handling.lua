local OBJ = RDV.SAL.AddSkill("Weapon Handling")

OBJ:SetMaxTiers(3)

OBJ:SetCategory("Perks")

OBJ:SetDescription("Improved accuracy, better stability.")
...
OBJ:AddHook("PlayerLoadout", function(ply)
    ...
    if TIER > 0 then
        ...
        local attachment
        if (TIER == 1) then
            attachment = "rc_perk_weapon_handling_1"
        elseif (TIER == 2) then
            attachment = "rc_perk_weapon_handling_2"
        else
            attachment = "rc_perk_weapon_handling_3"
        end
