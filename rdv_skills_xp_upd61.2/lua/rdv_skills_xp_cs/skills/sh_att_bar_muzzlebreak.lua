local OBJ = RDV.SAL.AddSkill("Muzzle Break")
...
OBJ:AddHook("PlayerLoadout", function(ply)
    ...
    if TIER > 0 then
        ...
        local attachment
        if (TIER == 1) then
            attachment = "dc17m_muzzle_muzzlebreak_1"
        elseif (TIER == 2) then
            attachment = "dc17m_muzzle_muzzlebreak_2"
        else
            attachment = "dc17m_muzzle_muzzlebreak_3"
        end
