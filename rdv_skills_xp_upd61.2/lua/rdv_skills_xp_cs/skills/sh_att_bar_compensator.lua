local OBJ = RDV.SAL.AddSkill("Compensator")

OBJ:SetMaxTiers(3)

OBJ:SetCategory("Barrel")

OBJ:SetDescription("Less recoil, less range.")
...
OBJ:AddHook("PlayerLoadout", function(ply)
    ...
    if TIER > 0 then
        ...
        local attachment
        if (TIER == 1) then
            attachment = "dc17m_muzzle_compensator_1"
        elseif (TIER == 2) then
            attachment = "dc17m_muzzle_compensator_2"
        else
            attachment = "dc17m_muzzle_compensator_3"
        end
