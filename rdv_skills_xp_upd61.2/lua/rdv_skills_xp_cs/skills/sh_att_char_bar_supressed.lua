local OBJ = RDV.SAL.AddSkill("Omega Supressor")

OBJ:SetMaxTiers(1)

OBJ:SetCategory("Character")

OBJ:SetDescription("Supressed gun.")
...
OBJ:SetNoEffectTeams({
    TEAM_RCNINER, TEAM_RCFI, TEAM_RCDARMAN, TEAM_RCATIN, TEAM_RCCORR
})
...
OBJ:AddHook("PlayerLoadout", function(ply)
    ...
    if TIER > 0 then
        ...
        local attachment
        if (TIER == 1) then
            attachment = "rc_omega_suppressor"
        end
