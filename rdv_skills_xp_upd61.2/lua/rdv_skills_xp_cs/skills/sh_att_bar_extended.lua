local OBJ = RDV.SAL.AddSkill("Extended Barrel")

OBJ:SetMaxTiers(3)

OBJ:SetCategory("Barrel")

OBJ:SetDescription("More damage, slower.")
...
OBJ:AddHook("PlayerLoadout", function(ply)
    ...
    if TIER > 0 then
        ...
        local attachment
        if (TIER == 1) then
            attachment = "dc17m_muzzle_extbarrel_1"
        elseif (TIER == 2) then
            attachment = "dc17m_muzzle_extbarrel_2"
        else
            attachment = "dc17m_muzzle_extbarrel_3"
        end
