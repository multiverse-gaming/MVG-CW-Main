local OBJ = RDV.SAL.AddSkill("Versatile")

OBJ:SetMaxTiers(3)

OBJ:SetCategory("Perks")

OBJ:SetDescription("Shoot better and aim better whilst moving.")
...
OBJ:AddHook("PlayerLoadout", function(ply)
    ...
    if TIER > 0 then
        ...
        local attachment
        if (TIER == 1) then
            attachment = "rc_perk_versatile_1"
        elseif (TIER == 2) then
            attachment = "rc_perk_versatile_2"
        else
            attachment = "rc_perk_versatile_3"
        end
