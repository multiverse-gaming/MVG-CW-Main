...
local droptables = {
	...
	["zombies"] = { "sfw_saphyre", "sfw_aquamarine" }
}
...
hook.Add( "OnNPCKilled", "DropItemOnNPCKilled", function( npc, attacker )
	...
	if ( SERVER ) then
		...
		local cmd_allowdrops = GetConVarNumber( "sfw_allow_supplydrop" )
		local cmd_dropchance_items = GetConVarNumber( "sfw_supplydrop_chance_items" )
		local cmd_dropchance_weapons = GetConVarNumber( "sfw_supplydrop_chance_weapons" )
		local cmd_debug_chattext = GetConVarNumber( "sfw_debug_obviousdropmessages" )
		local cmd_particles = GetConVarNumber( "sfw_fx_particles" ) 
	
		if ( cmd_allowdrops ~= 1 ) then return end

		local drop = table.Random( items ) --items[ math.random( 1, #items ) ]
