...
function SWEP:AddVStatsDisp()
	...
	if ( showstats > 0 ) then
		...
		cam.End2D()
--	else
--		showstats = Lerp( showstats, showstats, 0 )
	end

	showstats = math.Clamp( showstats - 1, 0, 100 )
	
end

function SWEP:AddRPGAcc()

	if ( self.Owner == NULL ) and ( self:GetNWBool( "MobDrop" ) == true || GetConVarNumber( "sfw_debug_force_itemhalo" ) == 1 ) then -- omg, dat borderlands ref --
