...
SWEP.Purpose				= "An Energy Sword."
SWEP.Instructions			= "Mouse1 to strike, Mouse2 to stab/dash. Hitting a target will decrease the blade's charge."

SWEP.Slot					= 0
SWEP.SlotPos				= 4

SWEP.ViewModel				= "models/weapons/w_knife_t.mdl"	-- Weapon view model
SWEP.WorldModel				= "models/doom/weapons/templar_sword.mdl"
SWEP.ShowWorldModel         = false
SWEP.HoldType 				= "bigsword"
...
SWEP.SciFiMeleeASpeed		= 0.15
SWEP.SciFiMeleeRange		= 100
SWEP.SciFiMeleeDamage		= 500
...
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/doom/weapons/templar_sword.mdl", bone = "v_weapon.knife_Parent", rel = "", pos = Vector(0.518, -0.519, 1.557), angle = Angle(0, 0, 0), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}



SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/doom/weapons/templar_sword.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3.635, 0.518, -1.558), angle = Angle(8.182, 75.973, 108.7), size = Vector(0.699, 0.699, 0.699), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

--resource.AddSingleFile( "weapons/eblade/eblade_idle.wav" )

local SwingElectric = { "hpwrewrite/wand/spellcast02.wav", "weapons/melee/rifle_swing_hit_world.wav", "doors/handle_pushbar_locked1.wav" }
local HitElectric = { "weapons/blades/clash.mp3", "weapons/blades/nastystab.mp3" }
local HitDefault = Sound( "buttons/lever8.wav" )
local comfort = 0
...
function SWEP:Deploy()
	...
	self.Weapon:SetHoldType( "bigsword" )
...
function SWEP:DealDamage()
	...
	if ( tr.Hit && !( game.SinglePlayer() && CLIENT ) ) then
		...
		if ( anim == "stab" ) then
			ParticleEffect( "purplecracker_main", tr.HitPos, self.Owner:EyeAngles(), self )
		else
			ParticleEffect( "purplecracker_main", tr.HitPos, self.Owner:EyeAngles(), self )
		end
	...
	if ( SERVER && IsValid( tr.Entity ) && ( tr.Entity:IsNPC() || tr.Entity:IsPlayer() || tr.Entity:Health() > 0 ) ) then
		...
		--	self:DealAoeDamage( bit.bor( DMG_AIRBOAT, DMG_NEVERGIB ), 25 * amp, self.Owner:GetShootPos() + self.Owner:GetAimVector() * 16, 64 )
			...
			dmginfo:SetDamage( ( math.random( 500, 800 ) + ( self:Clip1() / 4 ) ) * amp + velo )
			dmginfo:SetDamageType( bit.bor( DMG_ENERGYBEAM, DMG_NEVERGIB ) )
			self:TakePrimaryAmmo( math.Clamp( 25, 0, self:Clip1() ) )
		else
			dmginfo:SetDamage( math.random( 500, 800 ) * amp + velo )
		end
