...
SWEP.PrintName						= "DC-17m (DMR)"
SWEP.Type							= "Republic high-precision Blaster Rifle"
SWEP.DrawAmmo						= true
SWEP.data 							= {}
...
SWEP.Primary.DefaultClip			= 40*10
SWEP.Primary.RPM					= 450
SWEP.Primary.RPM_Burst				= nil
SWEP.Primary.Ammo					= "ar2"
SWEP.Primary.AmmoConsumption 		= 1
SWEP.Primary.Range 					= 52495*10
...
SWEP.ChangeStateRecoilMultiplier	= 1.0
SWEP.CrouchAccuracyMultiplier		= 0.34
SWEP.ChangeStateAccuracyMultiplier	= 1
SWEP.JumpAccuracyMultiplier			= 2
SWEP.WalkAccuracyMultiplier			= 0.4
...
SWEP.BlowbackVector 				= Vector(0,-1.5,-0.035)
...
SWEP.Primary.Spread					= 0.0153
SWEP.Primary.IronAccuracy 			= 0.00525
...
SWEP.IronSightsPos = Vector(-5.612, -15, 0.485)
...
SWEP.Attachments = {[1]={header="Modes",atts={"dc17m_railgun"},},
                    [2]={header="Barrel",atts={"raid_grapple"},},
					[3]={header="Barrel",atts={"dc17m_dmr"},},
}
...
SWEP.VElements = {
	...
	["rifle_mag"] = { type = "Model", model = "models/cs574/dc17m/dc17m_mag.mdl", bone = "", rel = "dc17m", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, active = false  },
	...
	["rifle_magdrum"] = { type = "Model", model = "models/cs574/dc17m/dc17m_drummag.mdl", bone = "", rel = "dc17m", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, active = true  },
...
if CLIENT then
	surface.CreateFont( "CustomFont", {font = "Aurebesh",extended = false,size = 200,weight = 0,blursize = 0,scanlines = 0,antialias = false,underline = false,italic = false,strikeout = false,symbol = false,rotary = false,shadow = false,additive = false,outline = false,})
end

local ReloadInSeconds = 5

SWEP.TimeStarted = CurTime()
SWEP.TimeEnded = nil
...
function SWEP:Think(...)
	...
	if  self:IsAttached("dc17m_railgun") then
		if self:Clip1() < self:GetMaxClip1() then

			if self:Clip1() == 0 and (self.TimeEnded == nil or self.TimeEnded < CurTime()) then
				self:SetClip1(1)
				self.TimeStarted = CurTime()
				self.TimeEnded = self.TimeStarted + ReloadInSeconds
			end



			local currentCharge = math.Clamp( ((CurTime() - self.TimeStarted)/ReloadInSeconds) * self:GetMaxClip1(), 0, self:GetMaxClip1())

			local rounded = math.Round(currentCharge, 0)

			if self:Clip1() != rounded then
				self:SetClip1(currentCharge)
			end
		end
	...
	if CLIENT then
		if !self:IsAttached("dc17m_railgun") then
			...
			if self:Clip1()>99 then self.VElements["dc17m_ammo"].draw_func=function(weapon)draw.SimpleText(""..weapon:Clip1(),"CustomFont",0,0,Color(0,120,180,255),TEXT_ALIGN_CENTER,TEXT_ALIGN_TOP)end end
			if self:Clip1()<=99 then self.VElements["dc17m_ammo"].draw_func=function(weapon)draw.SimpleText("0"..weapon:Clip1(),"CustomFont",0,0,Color(0,120,180,255),TEXT_ALIGN_CENTER,TEXT_ALIGN_TOP)end end
			if self:Clip1()<=9 then self.VElements["dc17m_ammo"].draw_func=function(weapon)draw.SimpleText("00"..weapon:Clip1(),"CustomFont",0,0,Color(0,120,180,255),TEXT_ALIGN_CENTER,TEXT_ALIGN_TOP)end end
			if self:Clip1()==0 then self.VElements["dc17m_ammo"].draw_func=function(weapon)draw.SimpleText("000","CustomFont",0,0,Color(0,120,180,255),TEXT_ALIGN_CENTER,TEXT_ALIGN_TOP)end end
			...
			if self:Clip1()==100 then
				self.VElements["sniper_mag"].pos=Vector(-2.45,0,0)
				self.VElements["sniper_magext"].pos=Vector(-4.08,0,0)
			else
				self.VElements["sniper_mag"].pos=Vector(-2.9,0,0)
				self.VElements["sniper_magext"].pos=Vector(-4.5,0,0)
			end
		end
	end
end
...
function SWEP:Deploy()
	...
	hook.Add("SetupMove", "Disable Reload ON DC-17m RailGun if using", function(ply, mvd, cmd)
		if mvd:KeyDown(IN_RELOAD) and ply:GetActiveWeapon():GetClass() == "rw_sw_dc17m_dmr" and ply:GetActiveWeapon():IsAttached("dc17m_railgun") then
			mvd:RemoveKeys(IN_RELOAD)
		end
	end)
end