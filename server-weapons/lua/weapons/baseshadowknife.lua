...
if ( CLIENT ) then
	SWEP.PrintName          = "Baseshadowknife"
	SWEP.DrawAmmo           = false
...
function SWEP:DoAttack( Altfire )
    ...
    if DidHit then
        if HitEntity and IsValid( HitEntity ) then
            Backstab = DidHitPlrOrNPC and self:EntityFaceBack( HitEntity ) -- Because we can only backstab creatures
						Damage = ( Altfire and (Backstab and 600) ) or ( Backstab and 600) or (Altfire and 600) or 600
    ...
    local NextAttack = Altfire and 10 or DidHit and 10 or 10
