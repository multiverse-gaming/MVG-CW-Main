...
function SWEP:DoHandcuff( target )
	...
	if (self:GetClass() == "weapon_leash_rope") then
		cuff.Leash = true
	end
	...
	cuff:SetIsUnbreakable( self.CuffUnbreakable )
	...
	if self.CuffsCanAutoArrest and self.Owner.isCP and target.arrest and self.Owner:isCP() and cvars.Bool("cuffs_autoarrest") then
...
function SWEP:DrawHUD()
	if not self:GetIsCuffing() then
		...
		draw.SimpleText( "Cooldown", "DermaDefault", w+1, h+31, Col.TextShadow, TEXT_ALIGN_CENTER )
		draw.SimpleText( "Cooldown", "DermaDefault", w, h+30, Col.Text, TEXT_ALIGN_CENTER )
	...
	draw.SimpleText( "Restraining target...", "DermaDefault", w+1, h+31, Col.TextShadow, TEXT_ALIGN_CENTER )
	draw.SimpleText( "Restraining target...", "DermaDefault", w, h+30, Col.Text, TEXT_ALIGN_CENTER )
