	...
	if SERVER and ply:HasWeapon("cloaking-3") then
		if cloakconfig.CloakMode == "Charge" and cloakconfig.MaxCharge3 ~= 0 then
			...
			if ply.LastCharge + (1 * cloakconfig.ChargeLossMultiplier) <= CurTime() and ply:GetNWFloat("CloakCharge") > 0 and ply.CloakActive then
				...
				ply.LastCharge = CurTime()
			elseif ply.LastCharge + (1 * cloakconfig.ChargeGainMultiplier) <= CurTime() and ply:GetNWFloat("CloakCharge") < cloakconfig.MaxCharge3 and not ply.CloakActive and not ply.CloakPause then
				-- Adds Charge
				ply:SetNWFloat("CloakCharge", ply:GetNWFloat("CloakCharge") + 1)
				ply.LastCharge = CurTime()
