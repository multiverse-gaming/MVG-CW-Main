if CLIENT then
	...
	SWEP.Slot = 4
	SWEP.SlotPos = 0
	SWEP.Description = "This stims adds 20% extra health from the base of the targets health. Abilitity to left click grants targets 150 HP extra each time to max extra health. Abilitity to right click grants 75 HP extra each time to max extra health."
	SWEP.Contact = ""
		...
		if targetPly:IsPlayer() then
			...
			if targetPly:Health() >= maxhealth then
				...
				if newHealth > maxhealth then
					targetPly:SetHealth(maxhealth)
				else
					targetPly:SetHealth(newHealth)
				end
...
function SWEP:SecondaryAttack()
	...
	local maxhealth = self.Owner:GetMaxHealth() + 100
