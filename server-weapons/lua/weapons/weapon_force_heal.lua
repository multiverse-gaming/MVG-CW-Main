...
function SWEP:PrimaryAttack()
	...
	if ( IsValid( ent ) ) then 
	need = math.min( ent:GetMaxHealth() - ent:Health(), ent:GetMaxHealth() * .25 ) 
	end

	if ( IsValid( ent ) && self:Clip1() >= 50 && ( ent:IsPlayer() || ent:IsNPC() ) && ent:Health() < ent:GetMaxHealth() ) then
		self:TakePrimaryAmmo( need )

		ent:SetHealth( math.min( ent:GetMaxHealth(), ent:Health() + 200 ) )
...
--[[function SWEP:SecondaryAttack()
	...
	if ( IsValid( ent ) && self:Clip1() >= 50 && ent:Health() < ent:GetMaxHealth() ) then
