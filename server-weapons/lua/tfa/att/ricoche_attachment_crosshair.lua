...
local config = {
	["DamageMultiplier"] = 1, // Damage multiplier to increase shot damage by
	["NumRicochet"] = 2, // Number of times the bullet will bounce off of walls / NPCs
	["Clipsize"] = 25 // Number of shots per clip
}
...
function ATTACHMENT:ShootBullet(damage, recoil, num_bullets, aimcone, disablericochet, bulletoverride)
	...
	self.MainBullet.Force	= damage/1 * math.sqrt((self.Primary.KickUp+self.Primary.KickDown+self.Primary.KickHorizontal )) * GetConVarNumber("sv_tfa_force_multiplier",1) * self:GetAmmoForceMultiplier()				-- Amount of force to give to phys objects
